...
public:
    ...
    using DepthCallback     = std::function<void(
        const HFTToolset::Symbol&, const std::vector<HFTToolset::BookLevel>& bids, const std::vector<HFTToolset::BookLevel>& asks )>;
...
private:
    ...
    void match_against( PriceLevels& levels,
                        const HFTToolset::Order& incoming,
                        HFTToolset::Quantity& remaining,
                        std::vector<HFTToolset::Trade>& trades,
